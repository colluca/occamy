...
def get_tests(testlist, run_dir, hw_cfg):
    ...
    for test in tests:
        ...
        axes = [
            test['tile'],
            common.get_mcast_prefix(test['multicast']),
        ]
        ...
        verify_py = str(common.TARGET_DIR / f'sw/device/apps/blas/{DEVICE_APP}/scripts/verify.py')
        test['cmd'] = [verify_py, '--symbols-bin', str(test['device_elf']), "${sim_bin}", "${elf}"]
